...
const (
	...
	// Explanation:
	//  * A breaking API change in Kurt must also yield a breaking API change in the module lib, since the
	...
	expectedMajorMinorVersion = "0.49"
	// !!!!!! BEFORE YOU UPDATE THIS CONSTANT TO FIX THE TEST, ADD A "BREAKING CHANGE" SECTION IN THE CHANGELOG !!!!!!
)

// This test ensures that when you bump to a Kurt version that has an API break, you're reminded to add a "Breaking Changes"
//
//	entry to the engine server's changelog as well (since a Kurt API break is an engine server API break)
func TestYouHaveBeenRemindedToAddABreakingChangelogEntryOnKurtAPIBreak(t *testing.T) {
	actualKurtCoreSemver, err := semver.Parse(kurtosis_version.KurtosisVersion)
	require.NoError(t, err, "An unexpected error occurred parsing Kurt Core version string '%v'", kurtosis_version.KurtosisVersion)
	actualCoreMajorMinorVersion := fmt.Sprintf("%v.%v", actualKurtCoreSemver.Major, actualKurtCoreSemver.Minor)
	require.Equal(t, expectedMajorMinorVersion, actualCoreMajorMinorVersion)
}
